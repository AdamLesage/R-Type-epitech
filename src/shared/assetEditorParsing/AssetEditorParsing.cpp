...
void AssetEditorParsing::parseSprite(EntityData& entityData, libconfig::Setting& components) {
    try {
        ...
        entityData.sprite =
            std::make_shared<Sprite>(Sprite{spritePath, {rectSizeX, rectSizeY}, {rectPosX, rectPosY}});
    } catch (const std::exception&) {
        return;
    }
}
...
void AssetEditorParsing::parseHealth(EntityData& entityData, libconfig::Setting& components) {
    try {
        ...
        entityData.health = std::make_shared<Health>(Health{static_cast<unsigned long>(health),
                                                            static_cast<unsigned long>(health),
                                                            (bool)isRegenerating, (bool)isDamageable});
    } catch (const std::exception&) {
        return;
    }
}
...
void AssetEditorParsing::parseType(EntityData& entityData, libconfig::Setting& components) {
    std::map<std::string, EntityType> entityMap = {{"PLAYER", PLAYER},
                                                   {"ENEMY", ENEMY},
...
void AssetEditorParsing::parseShootStraightPattern(EntityData& entityData, libconfig::Setting& components) {
    try {
        ...
        entityData.shootStraightPattern                 = std::make_shared<ShootStraightPattern>(
            ShootStraightPattern{projectileSpeed, shootCooldown, std::chrono::steady_clock::now()});
    } catch (const std::exception&) {
        return;
    }
}
...
void AssetEditorParsing::parseShootPlayerPattern(EntityData& entityData, libconfig::Setting& components) {
    try {
        ...
        entityData.shootPlayerPattern                 = std::make_shared<ShootPlayerPattern>(
            ShootPlayerPattern{projectileSpeed, shootCooldown, std::chrono::steady_clock::now()});
    } catch (const std::exception&) {
        return;
    }
}
...
void AssetEditorParsing::parsePlayerFollowingPattern(EntityData& entityData, libconfig::Setting& components) {
    try {
        ...
        entityData.playerFollowingPattern =
            std::make_shared<PlayerFollowingPattern>(PlayerFollowingPattern{speed});
    } catch (const std::exception&) {
        return;
    }
}
...
void AssetEditorParsing::parseWavePattern(EntityData& entityData, libconfig::Setting& components) {
    try {
        ...
        entityData.WavePattern                 = std::make_shared<Wave_pattern>(
            Wave_pattern{amplitude, frequency, std::chrono::steady_clock::now()});
    } catch (const std::exception&) {
        return;
    }
}
