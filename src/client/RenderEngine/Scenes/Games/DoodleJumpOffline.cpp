...
void RType::DoodleJumpOffline::handleOfflineGame() {
    ...
    for (size_t i = 0; i < this->_camera->listEntityToDisplay.size(); i++) {
        if (this->_camera->listEntityToDisplay[i].sprite.spritePath
                == std::string("assets") + PATH_SEPARATOR + "doodle_jump" + PATH_SEPARATOR + "lik-left.png"
            || this->_camera->listEntityToDisplay[i].sprite.spritePath
                   == std::string("assets") + PATH_SEPARATOR + "doodle_jump" + PATH_SEPARATOR
                          + "lik-right.png"
            || this->_camera->listEntityToDisplay[i].sprite.spritePath
                   == std::string("assets") + PATH_SEPARATOR + "doodle_jump" + PATH_SEPARATOR
                          + "lik-front.png") {
    ...
    if (!isPlayerInList) {
        ...
        entity.sprite.spritePath =
            std::string("assets") + PATH_SEPARATOR + "doodle_jump" + PATH_SEPARATOR + "lik-front.png";
    ...
    if (this->_camera->listEntityToDisplay.size() < NUMBER_PLATFORM) {
        for (size_t i = this->_camera->listEntityToDisplay.size(); i < NUMBER_PLATFORM; i++) {
            ...
            entity.size       = {100, 100};
            entity.sprite.spritePath =
                std::string("assets") + PATH_SEPARATOR + "doodle_jump" + PATH_SEPARATOR + "green-tile.png";
            entity.sprite.rectPos  = {0, 0};
            entity.sprite.rectSize = {100, 100};
            entity.direction.x     = 0;
    ...
    for (size_t i = 0; i < this->_camera->listEntityToDisplay.size(); i++) {
        ...
        if (this->_camera->listEntityToDisplay[i].sprite.spritePath
            ...
            if (this->_camera->listEntityToDisplay[i].direction.x > 0) {
                this->_camera->listEntityToDisplay[i].sprite.spritePath =
                    std::string("assets") + PATH_SEPARATOR + "doodle_jump" + PATH_SEPARATOR + "lik-right.png";
            ...
            if (this->_camera->listEntityToDisplay[i].position.y
                > 1080 + 1080 / 2) { // Game over because player fell
                this->gameOver();
            }
        }
...
void RType::DoodleJumpOffline::applyGravity() {
        ...
        for (auto& player : this->_camera->listEntityToDisplay) {
            if (player.sprite.spritePath
                ...
                for (auto& platform : this->_camera->listEntityToDisplay) {
                    if (platform.sprite.spritePath
                            == std::string("assets") + PATH_SEPARATOR + "doodle_jump" + PATH_SEPARATOR
                                   + "green-tile.png"
                        && player.position.y < 100) {
                        platform.direction.y =
                            GRAVITY
                            * 2; // Increase speed of platforms because player is jumping on top of the screen
                    } else if (platform.sprite.spritePath
                                   == std::string("assets") + PATH_SEPARATOR + "doodle_jump" + PATH_SEPARATOR
                                          + "green-tile.png"
                               && player.position.y < 500) {
                        platform.direction.y = GRAVITY; // Increase speed of platforms because player is
                                                        // jumping on top of the screen
                    } else if (platform.sprite.spritePath
                                   == std::string("assets") + PATH_SEPARATOR + "doodle_jump" + PATH_SEPARATOR
                                          + "green-tile.png"
                               && player.position.y >= 500) {
    ...
    for (auto& entity : this->_camera->listEntityToDisplay) {
        if (entity.sprite.spritePath
            ...
            // First jump quickly
            if (_isJumping) {
                if (_startOfJump < std::chrono::system_clock::now()
                                       - std::chrono::milliseconds(200)) { // Apply impulse for 200ms
                    entity.direction.y = jumpImpulseVel;
                } else if (_startOfJump
                           < std::chrono::system_clock::now()
                                 - std::chrono::milliseconds(100)) { // Apply acceleration for 100ms
...
void RType::DoodleJumpOffline::handleInput() {
    ...
    for (size_t i = 0; i < this->_camera->listEntityToDisplay.size(); i++) {
        if (this->_camera->listEntityToDisplay[i].sprite.spritePath
            ...
            float speed =
                std::min(maxSpeed, std::abs(distance) / 10.0f); // Adjust the divisor to control sensitivity
