...
Edition::Asset::Asset(int x, int y, std::string assetPath, size_t entityCode) {
    ...
    this->addComponent<Sprite>(
        Sprite{assetPath,
               {static_cast<int>(_spriteTexture->getSize().x), static_cast<int>(_spriteTexture->getSize().y)},
               {0, 0}});
...
void Edition::Asset::draw(sf::RenderWindow& window) {
    ...
    this->_sprite.setTextureRect(
        sf::IntRect(sprite.rectPos[0], sprite.rectPos[1], sprite.rectSize[0], sprite.rectSize[1]));
