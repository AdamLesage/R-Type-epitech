...
void Systems::check_entities_collisions(Registry& reg,
    ...
    if (playerTakeDamage && collisionX && collisionY) {
        std::cout << "Player take damage from "
                  << (entityType2->type == EntityType::ENEMY_PROJECTILE ? "projectile" : "enemy")
                  << std::endl;
        ...
        if (entityType2->type == EntityType::ENEMY_PROJECTILE) {
            logger.log(RType::Logger::RTYPEINFO, "Projectile deleted");
...
void Systems::collision_system(Registry& reg,
    ...
    for (size_t i = 0; i < positions.size() && i < sizes.size(); i++) { // Browse all entities
        ...
        if (entityPos && entitySize, entityType) { // Check if entity exists
            ...
            for (size_t j = i + 1; j < positions.size() && j < sizes.size();
                 ++j) { // Check collision with other entities
                ...
                if (entityPos2 && entitySize2) { // Check if entity 2 exists
                    check_entities_collisions(reg, i, &(*entityPos), &(*entitySize), j, &(*entityPos2),
                                              &(*entitySize2), logger, networkSender, &(*entityType),
                                              &(*entityType2));
                }
            }
        }
